...
public:
    ...
    bool isClicked(Vector2i mouse) { return shape.getGlobalBounds().contains((Vector2f)mouse); }
    void draw(RenderWindow &w) { w.draw(shape); w.draw(text); }
};
...
public:
    ...
    bool move(int r, int c, char p) {
        if (grid[r][c] == ' ') { grid[r][c] = p; return true; }
        return false;
    }
...
private:
    ...
    void randomMove(Board &b) {
        ...
        if (!free.empty()) { auto m = free[rand() % free.size()]; b.grid[m.x][m.y] = 'O'; }
    }
...
public:
    void playMove(Board &b, Difficulty diff) {
        int chance = rand() % 100;
        if (chance < diff) bestMove(b); else randomMove(b);
    }
};
...
class Game {
public:
    enum State { MENU, ENTER_IP, ENTER_NAME, LOBBY, PLAYING, GAME_OVER, WAITING_CONNECTION };
    ...
    Font font;
    Button btnAI, btnDiff, btnLocal, btnJoinTourney, btnConnect, btnPlayAgain, btnMainMenu;
    ...
    Game() {
        ...
        btnJoinTourney = Button({250, 45}, {75, startY + 180}, "Join Tournament", font);
    ...
    void handleText(Uint32 unicode) {
        if (state == ENTER_IP) {
            if (unicode == '\b' && !ipInput.empty()) ipInput.pop_back();
            else if (unicode == '\r' || unicode == '\n') state = ENTER_NAME;
            else if (unicode >= 32 && unicode < 128 && ipInput.size() < 15) ipInput += static_cast<char>(unicode);
        } else if (state == ENTER_NAME) {
            if (unicode == '\b' && !nameInput.empty()) nameInput.pop_back();
            else if (unicode == '\r' || unicode == '\n') joinServer();
            else if (unicode >= 32 && unicode < 128 && nameInput.size() < 10) nameInput += static_cast<char>(unicode);
        }
    }

    void joinServer() {
        if (nameInput.empty()) nameInput = "Player";
        mode = NET_CLIENT;
    ...
    void handleClick(Vector2i m) {
        if (state == MENU) {
            ...
            else if (btnJoinTourney.isClicked(m)) { state = ENTER_IP; }
        } 
        else if (state == ENTER_IP) {
            if (btnConnect.isClicked(m)) state = ENTER_NAME;
        }
        else if (state == ENTER_NAME) {
            ...
            if (m.x >= BOARD_OFFSET_X && m.x <= BOARD_OFFSET_X + 300 &&
                ...
                if (board.grid[y][x] == ' ') {
                    if (mode == NET_CLIENT) {
                        // In tournament, only send the move to the server. Do not draw it yet!
                        // The server will validate it and broadcast the BOARD_UPDATE back.
                        if (playerTurn) {
                            sf::Packet packet;
    ...
    void draw(RenderWindow &w) {
        ...
        if (state == MENU) {
            ...
            btnAI.draw(w); btnDiff.draw(w); btnLocal.draw(w); btnJoinTourney.draw(w);
            return;
        }

        if (state == ENTER_IP || state == ENTER_NAME) {
            string prStr = (state == ENTER_IP) ? "Enter Server IP Address:" : "Enter Your Name:";
            ...
            txt.setPosition(60, 225); w.draw(txt);
        ...
        if (state == WAITING_CONNECTION || state == LOBBY) {
            string msg = (state == WAITING_CONNECTION) ? "Connecting..." : 
                         "LOBBY\n\nWaiting for players...\n" + to_string(lobbyCount) + " / " + to_string(lobbyRequired);
        ...
        if (mode == NET_CLIENT) {
            ...
            turnText.setFillColor(playerTurn ? Color(0, 255, 150) : Color(200, 100, 100));
        ...
        if (state == GAME_OVER) {
            ...
            // In Tournaments, the server decides when you play again, so we hide the local Play Again button.
            if (mode != NET_CLIENT) {
...
int main() {
    RenderWindow window(VideoMode(WINDOW_W, WINDOW_H), "Tic Tac Toe Tournament Edition");
